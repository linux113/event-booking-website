...
| ----- | -------------- |
| [`docs/connect-free-supabase.md`](./docs/connect-free-supabase.md) | Creating the database on Supabase's free tier: the migrations in order (SQL editor or CLI), which key goes in which variable after the 2026 rename, the first admin account, replacing the seed data with your real event, free-tier limits and the 7-day pause |
...
| [`docs/one-shot-schema.sql`](./docs/one-shot-schema.sql) | The whole schema as one 373 KB statement batch in a single transaction, for a provider's SQL editor — the same result as `db:setup`, generated by `node scripts/build-one-shot-schema.mjs --seed`, and it records the same bookkeeping so a later `db:setup` skips it |
## Booking flow
