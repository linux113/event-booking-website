...
| `npm run db:setup`  | Apply the schema to a **hosted** PostgreSQL (Neon, or any Postgres): `DATABASE_URL='…' npm run db:setup -- --seed`. Applies `supabase/prelude.sql` first — the `auth.users` + `auth.uid()` shim a bare server lacks — then all 16 migrations in filename order, each in its own transaction, stopping at the first failure; then asserts the end state (table, policy and RLS counts, and that a stranger is refused on `bookings`). The connection string comes from the environment only, and the password in it is masked in every message |
| [`docs/setup-database.workflow.yml`](./docs/setup-database.workflow.yml) | A GitHub Actions workflow that runs `npm run db:setup` on a runner (secret `NEON_DATABASE_URL`, on-demand only) — for when the machine you are on cannot reach the database. Copy it to `.github/workflows/setup-database.yml` to enable it: GitHub refuses to let an app push workflow files without a `workflows` permission this project does not need |
